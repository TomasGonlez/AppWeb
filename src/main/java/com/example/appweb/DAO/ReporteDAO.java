...
public class ReporteDAO {
    ...
    //REVISAR PORQUE NO DEVUELVE LA LISTA CON TODOS LOS INGRSO/SALIDAS DE TODOS LOS USUARIOS DEL SISTEMA
    public List<RegistroPersona> obtenerRegistrosPorFecha(String desde, String hasta) throws Exception {
        List<RegistroPersona> listaTotal = new ArrayList<>();
        String sql = "SELECT u.rut, u.nombreCompleto, r.fecha, r.tipo_registro, r.hora " +
                "FROM REGISTRO r JOIN USUARIO u ON r.id_usuario = u.id_usuario " +
    ...
    public List<RegistroPersona> obtenerRegistrosDependencias() throws Exception {
        List<RegistroPersona> lista = new ArrayList<>();
        String sql = "SELECT u.nombreCompleto, r.rut, r.fecha, r.tipo_registro, r.hora " +
                 "FROM REGISTRO r " +
                 "JOIN USUARIO u ON r.rut = u.rut " +
                 "INNER JOIN ( " +
                 "    SELECT rut, MAX(TO_CHAR(fecha, 'YYYY-MM-DD') || ' ' || hora) AS ultima_fecha_hora " +
                 "    FROM REGISTRO " +
                 "    GROUP BY rut " +
                 ") ult ON (TO_CHAR(r.fecha, 'YYYY-MM-DD') || ' ' || r.hora) = ult.ultima_fecha_hora " +
                 "AND r.rut = ult.rut " +
